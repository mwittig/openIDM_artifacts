...
// A configuration for allowed HTTP requests. Each entry in the configuration
// contains a pattern
// to match against the incoming request ID and, in the event of a match, the
// associated roles,
// methods, and actions that are allowed for requests on that particular
// pattern.
//
...
// customAuthz: A custom function for additional authorization logic/checks
// (optional)
// excludePatterns: A comma separated list of patterns to exclude from the
// pattern match (optional)
//
// A single '*' character indicates all possible values. With patterns ending in
// "/*", the "*"
// acts as a wild card to indicate the pattern accepts all resource IDs "below"
// the specified
// pattern (prefix). For example the pattern "managed/*" would match
// "managed/user" or anything
// starting with "managed/". Note: it would not match "managed", which would
// need to have its
...
var httpAccessConfig = {
	"configs" : [
			...
			// openidm-admin can request nearly anything (except query
			// expressions
			// on repo endpoints)
			{
				"pattern" : "*",
			...
			// additional rules for openidm-admin that selectively enable
			// certain
			// parts of system/
			{
				"pattern" : "system/*",
				"roles" : "openidm-admin",
				"methods" : "create,read,update,delete,patch,query", // restrictions
				// on
				// 'action'
				"actions" : "",
				"customAuthz" : "disallowQueryExpression()"
			...
			{
				"pattern" : "policy/*",
				"roles" : "openidm-authorized", // openidm-authorized is
				// logged-in users
				"methods" : "read,action",
				"actions" : "*"
			},
			...
			// This rule is primarily controlled by the ownDataOnly function -
			// that
			// will only allow
			// access to the endpoint from which the user originates
			// (For example a managed/user with the _id of bob will only be able
			// to
			// access managed/user/bob)
			...
			// enforcement of which notifications you can read and delete is
			// done
			// within the endpoint
			{
